...
function App() {
  ...
  const [items, setItems] = useState([]);
  const [remainingAmount, setRemainingAmount] = useState(0);
  const [porterage, setPorterage] = useState(0);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(null);
  const [enterCount, setEnterCount] = useState(0);
  const [focusedItemId, setFocusedItemId] = useState(null);
  ...
  // New states for discount, note, and received amount
  ...
  const loadInitialData = useCallback(() => {
    ...
    try {
      ...
      if (loggedIn === "true") {
        ...
        const savedPorterage = localStorage.getItem("porterage");
        if (savedPorterage) setPorterage(parseFloat(savedPorterage) || 0);

        // Load new states from localStorage
        const savedDiscountType = localStorage.getItem("discountType");
        if (savedDiscountType) setDiscountType(savedDiscountType);

        const savedDiscountValue = localStorage.getItem("discountValue");
        if (savedDiscountValue) setDiscountValue(savedDiscountValue);

        const savedNote = localStorage.getItem("note");
        if (savedNote) setNote(savedNote);

        const savedReceivedAmount = localStorage.getItem("receivedAmount");
        if (savedReceivedAmount) setReceivedAmount(savedReceivedAmount);
      }
    } catch (err) {
      setError("Failed to load initial data");
    } finally {
  ...
  const saveToLocalStorage = () => {
    if (isLoggedIn) {
      try {
        ...
        localStorage.setItem("items", JSON.stringify(items));
        localStorage.setItem("remainingAmount", remainingAmount.toString());
        localStorage.setItem("porterage", porterage.toString());
  ...
  const calculateTotals = useCallback(() => {
    ...
    let calculatedTotalBill = totalAmount + remainingAmount + porterage;

    if (discountValue !== "") {
      const discount = parseFloat(discountValue) || 0;
      if (discountType === "percentage") {
        discountAmount = (calculatedTotalBill * discount) / 100;
      } else if (discountType === "fixed") {
        discountAmount = discount;
      }
      calculatedTotalBill -= discountAmount;
    }

    const payableAmount = calculatedTotalBill - (parseFloat(receivedAmount) || 0);


    return { totalAmount, totalQuantity, totalBill: calculatedTotalBill, discountAmount, payableAmount };
  }, [items, remainingAmount, porterage, discountType, discountValue, receivedAmount]);

  const { totalAmount, totalQuantity, totalBill, discountAmount, payableAmount } = calculateTotals();
  ...
  const generateInvoiceText = () => {
    ...
    text += `Remaining (Old Balance): ₹${remainingAmount.toFixed(2)}\n`;
    text += `Porterage: ₹${porterage.toFixed(2)}\n`;
    if (discountValue) {
    ...
    text += `Received Amount: ₹${(parseFloat(receivedAmount) || 0).toFixed(2)}\n`;
    text += `Payable Amount: ₹${payableAmount.toFixed(2)}\n`;
    if (note) {
      text += "--------------------------------------\n";
      text += `Note: ${note}\n`;
    }
