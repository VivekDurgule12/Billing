...
function App() {
  ...
  // Load saved data when logged in
  useEffect(() => {
    if (isLoggedIn) {
  ...
  const clearBill = () => {
    if (window.confirm('Are you sure you want to clear the bill?')) {
      setCustomer({ name: '', address: '', date: new Date().toISOString().split('T')[0] });
      setItems([]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-800 via-gray-900 to-teal-900 text-white p-4 sm:p-6">
      {isLoggedIn ? (
        <div className="container mx-auto max-w-4xl bg-gray-700 rounded-3xl shadow-2xl p-6 sm:p-8 border border-teal-600">
          ...
          <BillingSummary totalAmount={totalAmount} remainingAmount={remainingAmount} setRemainingAmount={setRemainingAmount} />

          <InvoiceGenerator customer={customer} items={items} totalAmount={totalAmount} remainingAmount={remainingAmount} />

          {/* Clear Bill Button */}
          <button
            onClick={clearBill}
            className="bg-red-600 text-white px-8 py-3 rounded-xl mt-4 w-full sm:w-1/3 mx-auto block hover:bg-red-700 hover:scale-105 transition-all duration-300 shadow-lg text-lg font-semibold"
          >
            Clear Bill
          </button>
        </div>
