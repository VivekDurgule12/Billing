...
function App() {
  ...
  const addItem = () => {
    ...
    if (lastItem && (!lastItem.name || !isValidNumber(lastItem.quantity) || !isValidNumber(lastItem.price))) {
      setError('Please complete the previous item (name, qty, price) before adding a new one.');
      return;
    }
    setItems([...items, { 
      id: Date.now(), 
  ...
  const calculateTotals = useCallback(() => {
    ...
    const total = validItems.reduce((sum, item) => sum + (Number(item.quantity) * Number(item.price)), 0);
    return { total };
  }, [items]);
  ...
  const generateInvoiceText = () => {
    ...
    text += 'Items:\n';
    items.forEach((item) => {
      text += `${item.name || 'Unnamed'} - Qty:${item.quantity || '0'} * ₹${item.price || '0'} - ₹${item.total.toFixed(2)}\n`;
    });
    text += '----------------------------------------\n';
  ...
  const clearBill = () => {
    if (window.confirm('Are you sure you want to clear the bill? This will reset all data.')) {
      setCustomer({
        name: '',
        email: '',
        address: '',
      ...
      {isLoggedIn ? (
        <div className="container mx-auto max-w-6xl bg-gray-800/80 backdrop-blur-md rounded-3xl shadow-2xl p-8 border border-teal-500/30">
          ...
          <div className="space-y-8">
            <CustomerForm customer={customer} setCustomer={setCustomer} onKeyPress={handleCustomerKeyPress} />
            <ItemList 
              items={items}
