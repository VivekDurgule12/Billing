...
function App() {
    ...
    const addItem = () => {
        ...
        if (items.length > 0) {
            ...
            if (!isValidNumber(lastItem.quantity) || lastItem.quantity === "" || Number(lastItem.quantity) === 0) {
                setError("Item quantity is required, must be a number, and must be greater than zero.");
                setFocusedItemId(lastItem.id);
                setFocusedField("quantity");
                return;
            }
    ...
    const handleBlur = (field, itemId) => {
        if (focusedItemId === itemId && focusedField === field) {
            updateItem(itemId, field, document.getElementById(`${itemId}-${field}`).value);
            saveToLocalStorage();
        }
    };
    ...
    const handlePrint = useCallback(() => {
        ...
        const customerName = customer?.name || "Customer";
        const invoiceDate = customer?.date || new Date().toISOString().split("T")[0];
